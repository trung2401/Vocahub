...
export default function DashboardPage(_props: Readonly<DashboardPageProps>) {
  ...
  return <AppShell><div className="content">
    ...
    {createDialogOpen && <div className="modal-backdrop dialog-center" role="presentation"><section className="confirm-dialog create-dialog" role="dialog" aria-modal="true" aria-labelledby="create-deck-title"><h2 id="create-deck-title">{copy.dashboard.createDeckTitle}</h2><form onSubmit={handleCreate}><div className="field"><label htmlFor="create-deck-name">{copy.dashboard.createDeckName}</label><input id="create-deck-name" value={deckName} onChange={(event) => setDeckName(event.target.value)} placeholder={copy.dashboard.createDeckPlaceholder} autoFocus /></div><div className="confirm-actions"><button className="button button-secondary" type="button" onClick={closeCreateDialog}>{copy.import.cancel}</button><button className="button button-primary" type="submit" disabled={!deckName.trim()}><Plus size={13} />{copy.dashboard.createDeckShort}</button></div></form></section></div>}
