...
export default function DashboardPage(_props: Readonly<DashboardPageProps>) {
  const { decks, summariesByDeck, createDeck, deleteDeck } = useApp();
  ...
  const summaries = decks.map((deck) => summariesByDeck[deck.id]).filter(Boolean);
  const due = summaries.reduce((total, summary) => total + summary.dueEntries, 0);
  const totalEntries = summaries.reduce((total, summary) => total + summary.totalEntries, 0);
  ...
  return <AppShell><div className="content dashboard-content">
    ...
    {decks.length === 0 ? <div className="empty-state dashboard-empty"><span className="empty-icon"><FileUp size={28} /></span><p className="eyebrow">Bắt đầu phiên học đầu tiên</p><h2>{copy.dashboard.emptyTitle}</h2><p>{copy.dashboard.emptyDescription}</p><div className="button-row"><Link className="button button-primary" href="/import"><FileUp size={14} />{copy.dashboard.import}</Link><button className="button button-secondary" type="button" onClick={openCreateDialog}><Plus size={14} />{copy.dashboard.createDeck}</button></div><Link className="muted-link" href="/import">{copy.dashboard.supportedFormat}</Link></div> : <><div className="stats-grid dashboard-stats"><div className="stat-card"><span className="stat-icon warning"><Target size={17} /></span><div><span className="stat-label">{copy.dashboard.reviewToday}</span><div className="stat-value">{due}</div><span className="stat-support">thẻ đang chờ bạn</span></div></div><div className="stat-card"><span className="stat-icon accent"><BookOpen size={17} /></span><div><span className="stat-label">{copy.dashboard.totalWords}</span><div className="stat-value">{totalEntries}</div><span className="stat-support">trong các deck</span></div></div></div><div className="section-heading"><div><p className="eyebrow">Thư viện học tập</p><h2>{copy.dashboard.decksTitle}</h2></div><button className="icon-button" type="button" aria-label="Làm mới" onClick={() => window.location.reload()}><RotateCcw size={14} /></button></div><div className="deck-grid">{decks.map((deck) => <DeckCard key={deck.id} deck={deck} summary={summariesByDeck[deck.id] ?? { deckId: deck.id, totalEntries: 0, dueEntries: 0, masteredEntries: 0, learningEntries: 0 }} onDelete={(target) => setDeleting({ id: target.id, name: target.name })} />)}<button className="create-deck-card" type="button" onClick={openCreateDialog}><Plus size={19} /><strong>{copy.dashboard.createDeckShort}</strong><span>{copy.dashboard.createHint}</span></button></div></>}
