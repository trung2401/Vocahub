...
export default function QuizPage({ params }: Readonly<QuizPageProps>) {
  const { decks, entriesByDeck, recordReview } = useApp(); const deck = decks.find((item) => item.id === params.deckId); const entries = useMemo(() => entriesByDeck[params.deckId] ?? [], [entriesByDeck, params.deckId]);
  ...
  const questions = useMemo(() => buildQuizQuestions(entries), [entries]); const question = questions[index];
  if (!deck) return <div className="study-page" />;
  if (!questions.length || !question) return <div className="study-page"><div className="study-topbar"><Link href={`/decks/${deck.id}`} aria-label={copy.study.exit}><ArrowLeft size={16} /></Link><span className="study-title">{deck.name}</span></div><div className="study-content"><div className="empty-state"><h2>{copy.errors.insufficientChoices}</h2><Link className="button button-secondary" href={`/decks/${deck.id}`}>{copy.study.viewDeck}</Link></div></div></div>;
  const choose = async (optionIndex: number) => { if (selected !== null || !question) return; setSelected(optionIndex); const correct = question.options[optionIndex] === question.correct; await recordReview({ entryId: question.entryId, rating: correct ? 'good' : 'again', mode: 'quiz' }); if (correct) setScore((value) => value + 1); else setWrong((value) => [...value, question.entryId]); };
